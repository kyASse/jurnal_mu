...
import { Copy, Check, ExternalLink, BookOpen, Fingerprint, ShieldCheck } from 'lucide-react';
        ...
        <Card className={cn('relative overflow-hidden border-border/80 shadow-xs transition-all hover:shadow-md', className)}>
            <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                    ...
                    {hasPrefix && (
                        <Badge
                            variant="outline"
                            className="border-emerald-200 bg-emerald-50 text-emerald-800 text-[11px] dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"
                        >
            ...
            <CardContent className="space-y-4">
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            ...
                            <TooltipContent side="top">
                                {copied ? 'Berhasil disalin ke clipboard' : 'Salin prefix ke clipboard'}
                            </TooltipContent>
                        </Tooltip>
                    </TooltipProvider>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-1 text-xs">
                    <div className="space-y-1 rounded-md bg-background/50 p-2 border border-border/50">
                    ...
                    <div className="space-y-1 rounded-md bg-background/50 p-2 border border-border/50">
                        ...
                        <p className="font-medium text-foreground text-xs truncate">
                            {hasPrefix ? 'Aktif di Crossref' : 'Menunggu Prefix'}
                        </p>
                    </div>
                </div>
