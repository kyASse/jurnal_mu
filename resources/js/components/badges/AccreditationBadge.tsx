...
import { Award, AlertCircle, AlertTriangle, CheckCircle } from 'lucide-react';
        ...
        <div className="flex flex-col gap-1">
            ...
            {showDetails && (
                <div className="text-xs space-y-0.5">
                    <div className="text-gray-700 font-medium">No. {number}</div>
                    {expiry_date && (
                        <div className={`
                            ${expiry_status === 'expired' ? 'text-red-600' : ''}
                            ${expiry_status === 'expiring_soon' ? 'text-yellow-700' : ''}
                            ${expiry_status === 'valid' ? 'text-gray-600' : ''}
                        `}>
