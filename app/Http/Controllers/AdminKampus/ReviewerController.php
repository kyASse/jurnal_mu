...
{
    /**
     * Display a listing of Reviewers in the university.
     *
     * @route GET /admin-kampus/reviewers
     *
     * @features List reviewers in Admin Kampus's university, filter by expertise/workload, search
     */
     ...
     *
     * @route POST /admin-kampus/reviewers
     *
     * @features Toggle user in the university to reviewer role
     */
    public function store(StoreReviewerRequest $request): RedirectResponse
    {
        ...
        // Ensure user belongs to Admin Kampus's university
        if ($user->university_id !== $authUser->university_id) {
            abort(403, 'Cannot manage users from other universities.');
        }
     ...
     *
     * @features Show reviewer profile with stats
     */
    public function show(Request $request, User $reviewer): Response
    {
        $this->authorize('view', $reviewer);

        $authUser = $request->user();

        // Ensure reviewer belongs to Admin Kampus's university
        if ($reviewer->university_id !== $authUser->university_id) {
            abort(403, 'Cannot view reviewers from other universities.');
        }
        ...
        return Inertia::render('AdminKampus/Reviewer/Show', [
            'reviewer' => $reviewer,
            'stats' => $stats,
     ...
     *
     * @route GET /admin-kampus/reviewers/{id}/edit
     *
     * @features Edit reviewer metadata
     */
    public function edit(Request $request, User $reviewer): Response
    {
        ...
        // Ensure reviewer belongs to Admin Kampus's university
        if ($reviewer->university_id !== $authUser->university_id) {
            abort(403, 'Cannot edit reviewers from other universities.');
        }

        $reviewer->load(['roles']);

        return Inertia::render('AdminKampus/Reviewer/Edit', [
            'reviewer' => $reviewer,
            'scientificFields' => ScientificField::all(['id', 'name']),
            'expertiseFields' => $reviewer->expertiseFields(),
        ]);
     ...
     *
     * @route PUT /admin-kampus/reviewers/{id}
     *
    ...
    public function update(UpdateReviewerRequest $request, User $reviewer): RedirectResponse
    {
        ...
        // Ensure reviewer belongs to Admin Kampus's university
        if ($reviewer->university_id !== $authUser->university_id) {
            abort(403, 'Cannot update reviewers from other universities.');
        }
        ...
        return redirect()->route('admin-kampus.reviewers.show', $reviewer)
     ...
     *
     * @route DELETE /admin-kampus/reviewers/{id}
     *
     * @features Remove reviewer role
     */
    public function destroy(Request $request, User $reviewer): RedirectResponse
    {
        ...
        // Ensure reviewer belongs to Admin Kampus's university
        if ($reviewer->university_id !== $authUser->university_id) {
            abort(403, 'Cannot remove reviewers from other universities.');
        }
        ...
        return redirect()->route('admin-kampus.reviewers.index')
     ...
     *
     * @route GET /admin-kampus/reviewers/{id}/stats
     *
     * @features Get detailed stats for reviewer
     */
    public function stats(Request $request, User $reviewer): JsonResponse
    {
        ...
        // Ensure reviewer belongs to Admin Kampus's university
        if ($reviewer->university_id !== $authUser->university_id) {
            abort(403, 'Cannot view stats from other universities.');
        }
     ...
     *
     * @route POST /admin-kampus/reviewers/{user}/toggle
     *
